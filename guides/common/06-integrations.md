...
Good Vibe Coding이 GitHub 저장소를 자동으로 만들거나 PR을 생성하려면 GitHub CLI(`gh`)가 필요합니다 (`config.github.enabled = true`로 활성화).
...
`gh` CLI가 설치 + 인증되어 있고 `config.github.enabled = true`이면 신규 프로젝트 진입 시:

- 비공개/공개 저장소 중 선택 가능
- 저장소 생성 → git init → 초기 커밋 → push 자동 처리
- 협업 모드(`autoCreatePR` 등) 활성화
...
`/gv "Supabase를 사용해서 인증과 데이터베이스를 구현해줘"`로 진입하면 plan 그래프가 자동으로:
...
`/gv:execute` 시 Supabase 관련 코드가 자동으로 생성됩니다:
...
내장 `next-app` 템플릿(`presets/templates/next-app`)을 사용하면 Vercel에 바로 배포할 수 있는 구조가 만들어집니다. 자연어 또는 CLI로 진입:

```
/gv "next-app 템플릿으로 시작"

# 또는 CLI 직접 호출 (JSON stdin)
