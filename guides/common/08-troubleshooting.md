...
SDK(`import { GoodVibe } from 'good-vibe'`)를 사용할 때 에러 유형이 두 가지로 나뉩니다:

- **AppError** (`err.code`가 `INPUT_ERROR`, `NOT_FOUND`, `SYSTEM_ERROR`): 입력 검증, 비즈니스 로직, LLM 호출 에러
- **OS/런타임 에러** (`err.code`가 `EACCES`, `ENOSPC` 등): `FileStorage`의 파일 I/O 에러, Hook 함수에서 발생한 예외

차이점:

| 항목           | CLI (슬래시 커맨드)          | SDK (프로그래밍 API)                               |
| -------------- | ---------------------------- | -------------------------------------------------- |
| 에러 표시      | stderr에 힌트 포함 자동 출력 | `AppError` 또는 OS 에러 throw — 직접 catch 필요    |
| 복구 힌트      | `ERROR_HINTS`로 자동 제공    | 에러 메시지만 포함 (힌트 없음)                     |
| 재시도         | 커맨드 재실행                | `try/catch` + 재호출                               |
| 상태 자동 저장 | `project.json` 자동          | `FileStorage` 사용 시에만 자동                     |
| 스토리지 에러  | 내부 처리                    | `FileStorage` I/O 에러가 그대로 전파됨 (아래 참고) |

자주 만나는 SDK 스토리지 에러:

| `err.code` | 원인                           | 해결                                    |
| ---------- | ------------------------------ | --------------------------------------- |
| `EACCES`   | 파일/디렉토리 권한 부족        | `chmod`로 쓰기 권한 부여                |
| `ENOSPC`   | 디스크 공간 부족               | 디스크 정리 후 재시도                   |
| (없음)     | `SyntaxError` — JSON 파싱 실패 | `project.json` 파일 확인/삭제 후 재실행 |
