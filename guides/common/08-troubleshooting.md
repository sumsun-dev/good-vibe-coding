...
> **팁:** 카테고리가 `security`이면 건너뛰지 마세요. `performance`나 `build`(환경 이슈)는 건너뛴 후 나중에 `good-vibe:modify`로 보완할 수 있습니다.
...
3. 에스컬레이션이 발생하면 "직접 지시"로 수정 방향을 구체적으로 전달하세요

되돌리기:

- `good-vibe:modify`는 기존 코드를 변경하므로, 실행 전에 git 커밋이 되어 있으면 `git diff`로 변경 사항을 확인하고 필요 시 되돌릴 수 있습니다
- GitHub 모드(`github.enabled = true`)를 사용하면 별도 브랜치에서 작업하므로 merge 전에 검토할 수 있습니다

예방: 큰 수정은 한 번에 하지 말고, 작은 단위로 나눠서 `good-vibe:modify`를 여러 번 실행하세요.
...
| 메시지 예시                                          | 원인                           | 해결                                                       |
| ---------------------------------------------------- | ------------------------------ | ---------------------------------------------------------- |
| `다음 필드가 필요합니다: name`                       | 프로젝트 이름 미입력           | `good-vibe:new`에서 프로젝트 설명 입력                     |
