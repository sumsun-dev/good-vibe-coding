...
`Discusser`와 `Executor`를 직접 사용하면 `GoodVibe` 클래스에서 노출하지 않는 고급 옵션을 사용할 수 있습니다.
...
const discusser = new Discusser({
  ...
  parallelTiers: true, // 전체 에이전트 병렬 분석 (기본값: true)
  reviewModel: 'claude-haiku-4-5', // 리뷰용 별도 모델 지정
});
...
const executor = new Executor({
  ...
  enableCrossModel: true, // Gemini CLI로 교차 리뷰
  worktreeIsolation: true, // Phase별 git worktree 격리
  projectDir: '/path/to/project', // 프로젝트 디렉토리 지정
});
...
| 옵션            | 타입      | 기본값 | 설명                                           |
| --------------- | --------- | ------ | ---------------------------------------------- |
| `parallelTiers` | `boolean` | `true` | 전체 에이전트 병렬 분석 (`false`: Tier별 순차) |
| `reviewModel`   | `string`  | -      | 리뷰 단계에 사용할 별도 모델                   |

### Executor 고급 옵션

| 옵션                | 타입                     | 기본값  | 설명                      |
| ------------------- | ------------------------ | ------- | ------------------------- |
| `maxSteps`          | `number`                 | `500`   | 세션당 에이전트 호출 상한 |
| `enableCrossModel`  | `boolean`                | `false` | Gemini CLI로 교차 리뷰    |
| `providerConfig`    | `ProviderConfig \| null` | `null`  | 멀티 프로바이더 설정      |
| `messageBus`        | `object \| null`         | `null`  | 에이전트 간 메시지 버스   |
| `worktreeIsolation` | `boolean`                | `false` | Phase별 git worktree 격리 |
| `projectDir`        | `string \| null`         | `null`  | 프로젝트 디렉토리 경로    |

### `onReviewIntervention` Hook

interactive 모드에서 리뷰 결과를 보고 CEO가 개입할 수 있습니다:

```javascript
const result = await gv.execute(plan, {
  onReviewIntervention: async (step) => {
    console.log(`Phase ${step.phase} 리뷰 완료`, step.reviews);
    // 'proceed': 그대로 진행, 'revise': 수정 지침과 함께 재실행
    return { decision: 'proceed' };
    // 또는: return { decision: 'revise', revisionGuidance: '인증 로직 재검토' };
  },
});
```
...
`discuss()` 반환값의 `convergence.reason` 가능 값:

| reason          | 설명                         |
| --------------- | ---------------------------- |
| `threshold-met` | 승인율이 임계값(80%) 이상    |
| `max-rounds`    | 최대 라운드 도달 (수렴 못함) |
| `stagnation`    | 라운드 간 개선 없음 (정체)   |
