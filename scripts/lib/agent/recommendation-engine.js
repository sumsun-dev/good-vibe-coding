...
/** 조사로만 사용되는 1자 한글 (false positive 방지) */
const SINGLE_CHAR_PARTICLES = new Set(['은', '는', '이', '가', '을', '를', '의', '에', '로', '와', '과', '도', '만']);

/**
 * 설명 텍스트에서 키워드를 추출한다.
 * 공백/구두점 기준 토큰화, 한국어 조사 제거, 영어 소문자 정규화.
 * 한글 토큰은 1자도 허용 (웹, 앱, 봇 등), 단 조사 1글자는 필터링.
...
export function extractKeywords(description) {
  ...
  const tokens = description
    ...
    .filter((t) => {
      ...
      // 1자 한글: 조사로만 쓰이는 글자는 제외
      if (HAS_HANGUL.test(t) && !SINGLE_CHAR_PARTICLES.has(t)) return true;
      return false;
    });
