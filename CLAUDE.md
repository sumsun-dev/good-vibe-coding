...
3. **고급 12개** (커스터마이징, 수동 설정, 평가):
   `/new-project`, `/projects`, `/onboarding`, `/my-config`, `/add-skill`, `/add-agent`, `/persona`, `/edit-persona`, `/scaffold`, `/preset`, `/reset`, `/eval`
...
- `execution-verifier.js` — **v4.1** 실행 검증 + 구체화 (빌드 검증 → 프로젝트 기록 통합)
...
- `cli.js` — CLI 브릿지 (커맨드 → 라이브러리, 87개 커맨드)
