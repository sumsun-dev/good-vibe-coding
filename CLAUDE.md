...
- `task-graph-actions.js` — **state→action 매핑 + LLM builder** (ask/review/research/code는 buildXxxActions로 callLLM DI 실제 LLM 호출, plan은 placeholder 유지(B-4d). code는 happy path 5개 state만 LLM, fixing/escalating은 placeholder(B-4c-2). `defaultActions(taskType, { useLLM, callLLM })`로 LLM/placeholder 모드 분기)
