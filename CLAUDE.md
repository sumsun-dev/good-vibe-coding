...
## 핵심 설계: CLI-as-API 패턴
- `cli.js`의 104개 커맨드는 **에이전트용 내부 API** — 사용자가 직접 호출하지 않음
- 사용자는 `/hello`, `/new`, `/discuss` 등 **슬래시 커맨드**만 사용
- 슬래시 커맨드 → 에이전트 디스패치 → cli.js 내부 API 호출 → Core Library 실행
