...
- `task-graph-actions.js` — **state→action 매핑 + LLM builder** (ask/review/research는 buildXxxActions로 callLLM DI 실제 LLM 호출, code/plan은 placeholder 유지(B-4c/d). `defaultActions(taskType, { useLLM, callLLM })`로 LLM/placeholder 모드 분기)
