...
## 핵심 설계: CLI-as-API 패턴
- `cli.js`는 **경량 라우터** — 14개 핸들러 모듈(`scripts/handlers/*.js`)로 커맨드를 lazy-load 디스패치
...
- **스킬/에이전트 자동 추천**: /new 플로우에서 프로젝트 컨텍스트 기반 스킬/에이전트 추천 → 설치
...
- `code-materializer.js` — 코드 구체화 (마크다운 → 실제 파일 기록, path traversal 방지)
- `execution-verifier.js` — 실행 검증 + 구체화 (다언어 빌드 검증: Node/Python/Go/Java → 프로젝트 기록 통합)
- `orchestrator.js` — 멀티에이전트 오케스트레이션 (tier별 병렬 디스패치, 수렴 확인)
- `review-engine.js` — 크로스 리뷰 시스템 (리뷰어 선정, 품질 게이트)
- `complexity-analyzer.js` — 프로젝트 복잡도 분석 (모드/팀 규모/모델 티어 추천)
- `report-generator.js` — 보고서 생성
- `agent-feedback.js` — 에이전트 피드백 시스템 (프로젝트 결과 분석 → 사용자/프로젝트 레벨 오버라이드 저장)
- `agent-instruction-extractor.js` — 에이전트 지시사항 추출 (마크다운 파싱)
- `agent-optimizer.js` — 에이전트 중복/효율 최적화 (유사도 분석, 기여도 추적, 최적 팀 추천)
- `project-metrics.js` — 관측성 (비용/토큰 추적, 에이전트 기여도, 대시보드)
- `template-scaffolder.js` — 프로젝트 템플릿 스캐폴딩 (5개 built-in + custom)
- `template-engine.js` — Handlebars 템플릿 엔진 (커스텀 헬퍼 포함)
- `json-parser.js` — 3-tier LLM JSON 응답 파싱 유틸리티
- `domain-parsers.js` — 도메인별 JSON 응답 파서 + 스키마 검증 (리뷰, 복잡도, 태스크, 제안)
- `dispatch-plan-generator.js` — 구조화된 JSON 디스패치 계획 생성
- `validators.js` — 공통 입력 검증 유틸리티 + AppError 클래스 (inputError/notFoundError/systemError)
- `config.js` — 중앙 설정 모듈 (Object.freeze, 매직 넘버 제거)
- `schema-validator.js` — 경량 스키마 검증 (validate/coerce, 외부 의존성 0)
- `execution-loop.js` — 실행 상태 머신 (시맨틱 검증, 저널 기록, 부실 실행 감지)
- `recommendation-engine.js` — 스킬/에이전트 추천 엔진 (멀티시그널 스코어링, LLM 호출 없음)
- `setup-installer.js` — 스킬/에이전트 설치 모듈 (카탈로그 항목 → ~/.claude/ 설치)
- `auth-manager.js` — 멀티프로바이더 인증 관리 (크레덴셜 CRUD, 프로바이더 설정)
- `llm-provider.js` — 멀티모델 LLM 프로바이더 추상화 (Claude/OpenAI/Gemini, 네이티브 fetch)
- `gemini-bridge.js` — Gemini CLI 서브프로세스 래퍼 (shell injection 방지)
- `cross-model-strategy.js` — 크로스 모델 리뷰 라우팅 (구현자와 다른 모델 리뷰어 할당)
- `eval-engine.js` — A/B 평가 프레임워크 (멀티에이전트 vs 단순 접근법 비교)
- `app-paths.js` — 애플리케이션 경로 중앙 관리 (pluginRoot, claudeDir, userSkillsDir, userAgentsDir 포함)
- `prompt-builder.js` — 프롬프트 구성 유틸리티 (섹션 조합, 목록 변환)
- `file-writer.js` — 파일 시스템 유틸리티 (ensureDir, fileExists, readJsonFile, listFilesByExtension)
- `preset-loader.js` — 프리셋 JSON 로딩 (roles, stacks, workflows)
- `cache.js` — 지연 로딩 캐시 유틸리티
- `cli.js` — CLI 라우터 (14개 핸들러 모듈로 lazy-load 디스패치)
- `cli-utils.js` — CLI 유틸리티 (readStdin, output, outputOk, parseArgs)
- `handlers/*.js` — 14개 커맨드 핸들러 모듈 (project, team, discussion, execution, review, build, eval, auth, feedback, infra, metrics, template, task, recommendation)
