...
- `task-graph-runner.js` — **v2 그래프 드라이버** (selectGraph + actions 매핑으로 state machine 진행, onProgress/journal 콜백, maxSteps 무한 루프 방지, action 예외/잘못된 event 안전 처리)
- `task-graph-actions.js` — **state→action 매핑 + LLM builder** (5개 작업 유형 모두 buildXxxActions로 callLLM DI 실제 LLM 호출. plan executing은 `runGraph`로 code 서브그래프 위임(`SUBGRAPH_MAP['plan:executing'] = 'code'`). code는 fix-loop(fixing) + escalating(LLM이 CONTINUE/SKIP/ABORT 결정) 포함, ctx.fixAttempt mutable 누적으로 graph guard 동작. `defaultActions(taskType, { useLLM, callLLM })`로 LLM/placeholder 모드 분기)
...
- `budget-store.js` — **opt-in 예산 임계 영속 저장** (`{baseDir}/budget.json`, maxCostUsd/maxTokens, null로 개별 해제, 음수/NaN 거부, /gv:cost 핸들러용)
...
- `handlers/*.js` — 18개 핸들러: project, team, discussion, execution, review, build, eval, auth, feedback, infra, metrics, template, task, recommendation, learn, dispatch(v2 단일 진입점 라우팅), cost(v2 예산 임계 opt-in), gv-execute(v2 그래프 실행 골격)
