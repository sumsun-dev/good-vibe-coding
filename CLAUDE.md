...
- `recommendation-engine.js` — **v4.6** 스킬/에이전트 추천 엔진 (멀티시그널 스코어링, LLM 호출 없음)
- `setup-installer.js` — **v4.6** 스킬/에이전트 설치 모듈 (카탈로그 항목 → ~/.claude/ 설치)
- `app-paths.js` — **v4.4** 애플리케이션 경로 중앙 관리 (v4.6: claudeDir, userSkillsDir, userAgentsDir 추가)
- `prompt-builder.js` — **v4.4** 프롬프트 구성 유틸리티 (섹션 조합, 목록 변환)
- `cache.js` — **v4.4** 지연 로딩 캐시 유틸리티
- `cli.js` — **v4.6** CLI 라우터 (14개 핸들러 모듈로 lazy-load 디스패치)
- `cli-utils.js` — **v4.5** CLI 유틸리티 (readStdin, output, outputOk, parseArgs)
- `handlers/*.js` — **v4.6** 14개 커맨드 핸들러 모듈 (project, team, discussion, execution, review, build, eval, auth, feedback, infra, metrics, template, task, recommendation)
...
## v4.6 변경사항
- **스킬/에이전트 자동 추천**: `/new` 플로우 Step 4.5에서 프로젝트 컨텍스트 기반 추천 → 설치
- **추천 카탈로그**: `presets/recommendation-catalog.json` — 4개 스킬 + 8개 에이전트 카탈로그
- **추천 엔진**: `recommendation-engine.js` — 4가지 시그널(타입/복잡도/키워드/역할) 가중 스코어링
- **설치 모듈**: `setup-installer.js` — 카탈로그 항목을 `~/.claude/`에 설치, 중복 방지
- **CLI 확장**: recommendation 핸들러 추가 (recommend-setup, install-setup, list-installed, recommendation-catalog)
- **에이전트 플로우 통합**: `commands/new.md` Step 4.5, `add-skill.md`/`add-agent.md` 카탈로그 기반으로 교체
