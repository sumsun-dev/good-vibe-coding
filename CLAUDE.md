...
- `task-graph-actions.js` — **state→action 매핑 + LLM builder** (5개 작업 유형 모두 buildXxxActions로 callLLM DI 실제 LLM 호출 가능. plan executing은 `runGraph`로 code 서브그래프 위임(`SUBGRAPH_MAP['plan:executing'] = 'code'`). code는 happy path 5개 state만 LLM, fixing/escalating은 placeholder(B-4c-2). `defaultActions(taskType, { useLLM, callLLM })`로 LLM/placeholder 모드 분기)
