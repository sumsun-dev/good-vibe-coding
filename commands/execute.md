...
`approved`, `executing`, `reviewing` 상태인 프로젝트를 대상으로 합니다.

- **approved** → 새 실행 (Step 1.2로 진행)
- **executing / reviewing** → AskUserQuestion으로 재개 여부 확인:
  ```
  질문: "이전 실행을 이어서 할까요?"
  header: "재개"
  options:
    - label: "이어서 실행 (Recommended)"
      description: "중단된 Phase부터 재개합니다"
    - label: "처음부터 다시"
      description: "새로 실행을 초기화합니다 (기존 진행 초기화)"
  ```
  "이어서 실행" → `init-execution`에 `resume: true` 전달
  "처음부터 다시" → 상태를 `approved`로 변경 후 Step 1.2로 진행
...
### 1.2.5 팀 커뮤니케이션 설정 (plan-execute / plan-only만)

> 실행 모드 선택 후, 실행 초기화 전에 한 번만 확인합니다.
> **quick-build 프로젝트는 이 단계를 건너뜁니다** (팀 규모가 작아 효과 없음).
...
`project.mode`가 `plan-execute` 또는 `plan-only`인 경우에만 AskUserQuestion:

```
질문: "팀 커뮤니케이션을 활성화할까요?"
header: "커뮤니케이션"
options:
  - label: "기본 실행 (Recommended)"
    description: "빠르고 저렴한 기본 실행"
  - label: "팀 커뮤니케이션 ON"
    description: "리뷰 대화 + 전문가 협의 활성화 (품질↑, LLM 호출 추가)"
```

- **"기본 실행"** → `messaging = false` (기본값 유지)
- **"팀 커뮤니케이션 ON"** → `messaging = true`

선택 결과를 `init-execution`에 `messaging` 필드로 전달합니다.

### 1.2.6 멀티 AI 리뷰 설정 (선택)

> 실행 모드 선택 후, 실행 초기화 전에 한 번만 확인합니다.

`skills/multi-review/SKILL.md`의 **셋업** 플로우를 실행합니다.

이미 `reviewStrategy === 'cross-model'`이면 실행 준비 메시지에 `리뷰 방식: 멀티 AI` 한 줄만 추가하고 넘어갑니다.
...
### 2.1 Phase 실행 (Task tool — Tier 병렬 디스패치)

> **병렬 실행 원칙:** `execution-plan-with-reviews`의 `parallelGroups` (예: `[[1, 2], [3], [4, 5]]`)를 기반으로
> 같은 Tier의 Phase들은 **동시에 여러 Task tool로 병렬 실행**합니다 (Phase당 1개 Task tool).
> Tier 내 모든 Phase가 완료되면 다음 Tier로 진행합니다.

**디스패치 규칙:**

- `parallelGroups`가 없거나 모든 Tier가 1개 Phase면 → 기존 순차 실행
- 같은 Tier에 2개 이상 Phase가 있으면 → **병렬 Task tool 동시 실행**
- 각 Task tool은 `--phase N`으로 자신의 Phase를 명시
- `project.worktreeIsolation = true`이면 병렬 Phase별 독립 worktree 사용

**모드별 동작:**

- **인터랙티브:** Tier 단위로 병렬 실행 → Tier 완료 후 CEO에게 확인
- **배치 실행:** Tier 단위로 병렬 실행
- **자동:** Tier 단위로 병렬 실행 (에스컬레이션 시에만 중단)

**Task tool 프롬프트 (각 Phase — Tier 내 병렬 실행 시 Phase당 1개):**

```
프로젝트 ID: {ID}
실행 모드: {mode}
워크트리 격리: {worktreeIsolation} (project.worktreeIsolation — true이면 Phase별 워크트리 사용)
병렬 실행: {parallelGroups가 있고 현재 Tier에 2개 이상 Phase가 있으면 true}
대상 Phase: {N} (이 Task tool이 담당하는 Phase 번호)

{첫 Phase이고 실행 초기화가 필요한 경우 (initRequired):}
**실행 초기화 (Phase 실행 전에 수행):**
1. echo '{"id":"{ID}","mode":"{mode}","messaging":{messaging}}' | node ${CLAUDE_PLUGIN_ROOT}/scripts/cli.js init-execution
   (재개 시: echo '{"id":"{ID}","mode":"{mode}","messaging":{messaging},"resume":true}' | ...)
2. echo '{"id":"{ID}","status":"executing"}' | node ${CLAUDE_PLUGIN_ROOT}/scripts/cli.js update-status
3. node ${CLAUDE_PLUGIN_ROOT}/scripts/cli.js execution-plan-with-reviews --id {ID}
   → `parallelGroups` (예: `[[1,2],[3],[4,5]]`)를 확인하여 Tier별 병렬 디스패치에 사용
→ 초기화 결과(총 Phase 수, 태스크 수, parallelGroups)를 반환에 포함하세요.

Phase {N}의 실행 루프를 수행하고, 완료 시 결과 요약만 반환하세요:

**Phase 실행 루프:**

1. `node ... next-step --id {ID} --phase {N}` → Phase {N}의 현재 action 확인
   (병렬 실행 시 `--phase {N}`으로 특정 Phase의 상태를 조회)
2. action별 처리:
   - execute-tasks: 태스크 병렬 실행 (팀원별 Task tool 호출 또는 Gemini CLI)
     → `project.worktreeIsolation = true`이면 Phase별 worktree 생성 (create-worktree)
   - materialize: 코드 파일 생성 (is-code-task → verify-and-materialize, worktreePath 전달)
     → materialize 완료 후: echo '{"id":"{ID}","status":"reviewing"}' | node ${CLAUDE_PLUGIN_ROOT}/scripts/cli.js update-status
   - review: 팀 검토 (select-reviewers → resolve-review-assignments → 병렬 리뷰)
   - quality-gate: 품질 검증 (enhanced-quality-gate)
     → quality-gate 완료 후: echo '{"id":"{ID}","status":"executing"}' | node ${CLAUDE_PLUGIN_ROOT}/scripts/cli.js update-status
   - fix: 수정 (revision-prompt → 재실행)
   - commit: Phase 커밋 (commit-phase)
   - build-context: Phase 컨텍스트 생성 (save-task-output → build-phase-context)
     → worktree 사용 시 머지+정리 (remove-worktree --merge)
   - confirm-next-phase: (메인 세션에 반환)
   - escalate: (메인 세션에 에스컬레이션 정보 반환)
3. 각 action 완료 후 advance-execution CLI 호출로 상태 전이
   (병렬 실행 시 stepResult에 `"phase": {N}` 포함)
   `echo '{"id":"{ID}","stepResult":{"completedAction":"...","phase":{N}}}' | node ... advance-execution`
...
```
프로젝트 ID: {ID}
워크트리 격리: {worktreeIsolation} (project.worktreeIsolation)
시작 Phase: {startPhase}
종료 Phase: {endPhase} (또는 에스컬레이션 발생까지)

{첫 Phase이고 실행 초기화가 필요한 경우 (initRequired):}
**실행 초기화 (Phase 실행 전에 수행):**
1. echo '{"id":"{ID}","mode":"{mode}"}' | node ${CLAUDE_PLUGIN_ROOT}/scripts/cli.js init-execution
2. echo '{"id":"{ID}","status":"executing"}' | node ${CLAUDE_PLUGIN_ROOT}/scripts/cli.js update-status
3. node ${CLAUDE_PLUGIN_ROOT}/scripts/cli.js execution-plan-with-reviews --id {ID}
...
1. `node ${CLAUDE_PLUGIN_ROOT}/scripts/cli.js next-step --id {ID} --phase {N}` → Phase {N}의 현재 action 확인
   (병렬 실행 시 `--phase {N}`으로 특정 Phase의 상태를 조회)
2. action별 처리 (아래 Action 레시피)
3. `echo '{"id":"{ID}","stepResult":{"completedAction":"...","phase":{N},...}}' | node ${CLAUDE_PLUGIN_ROOT}/scripts/cli.js advance-execution` → 상태 전이
   (병렬 실행 시 stepResult에 `"phase": {N}` 포함하여 해당 Phase의 상태만 전이)
4. action이 complete, confirm-next-phase, escalate가 될 때까지 반복

**Action 레시피 (action → CLI 호출 → advance stepResult):**

> 모든 CLI: `node ${CLAUDE_PLUGIN_ROOT}/scripts/cli.js {명령}`

[execute-tasks] 태스크 실행
- 워크트리 격리 (`project.worktreeIsolation = true`인 경우):
  `echo '{"projectId":"{ID}","phase":{N},"branchName":"gv-phase-{N}-{slug}"}' | node ... create-worktree`
  → `{success, worktreePath}` — materialize/commit에서 사용. 실패 시 projectDir 그대로 사용 (graceful degradation)
- 코드 판별: `echo '{"task":{task}}' | node ... is-code-task` → `{isCodeTask: bool}`
- 프롬프트:
  코드: `echo '{"task":{task},"teamMember":{member}}' | node ... tdd-execution-prompt`
  비코드: `echo '{"task":{task},"teamMember":{member},"context":{ctx}}' | node ... execution-prompt`
- LLM으로 실행 → 결과 저장:
  `echo '{"id":"{ID}","taskId":"{taskId}","output":"..."}' | node ... save-task-output`
...
- `echo '{"taskOutput":"...","task":{task},"projectDir":"{dir}","worktreePath":"{worktreePath || null}"}' | node ... verify-and-materialize`
  → `{verified: bool, files: [...]}` (worktreePath가 있으면 해당 경로에 파일 생성)
- 결과 저장: `echo '{"id":"{ID}","taskId":"{taskId}","materializeResult":{result}}' | node ... add-task-materialization`
- 상태 변경: `echo '{"id":"{ID}","status":"reviewing"}' | node ... update-status`
- advance: `{"completedAction":"materialize"}`

[review] 팀 검토
- 리뷰어 선정: `echo '{"task":{task},"team":[...]}' | node ... select-reviewers` → `{reviewers:[...]}`
- 프로바이더 배정: `echo '{"reviewers":[...]}' | node ... resolve-review-assignments`
- 리뷰 프롬프트 + LLM: `echo '{"reviewer":{r},"task":{task},"taskOutput":"..."}' | node ... task-review-prompt`
- 결과 저장: `echo '{"id":"{ID}","taskId":"{taskId}","reviews":[...]}' | node ... add-task-reviews`
- advance: `{"completedAction":"review"}`

[quality-gate] 품질 검증
- `echo '{"reviews":[...],"executionResult":{materializeResult}}' | node ... enhanced-quality-gate`
  → `{passed: bool, criticalCount, importantCount, summary}`
- 상태 변경: `echo '{"id":"{ID}","status":"executing"}' | node ... update-status`
- advance: `{"completedAction":"quality-gate","qualityGateResult":{"passed":bool,"issues":[...]}}`
  → passed=true → commit, passed=false+fixAttempt<2 → fix, 그 외 → escalated

[fix] 수정
- 컨텍스트: `node ... get-failure-context --id {ID}` → `{failureContext, failureHistory}`
- 프롬프트: `echo '{"task":{task},"implementer":{m},"reviews":[...],"failureContext":{ctx}}' | node ... revision-prompt` → LLM
- 결과 저장: save-task-output
- advance: `{"completedAction":"fix"}` → 다음: materialize (재진입)

[commit] Phase 커밋
- `echo '{"projectDir":"{worktreePath || dir}","phase":{N}}' | node ... commit-phase`
  (infraPath 없으면 건너뛰고 바로 advance, worktreePath가 있으면 worktree에서 커밋)
- advance: `{"completedAction":"commit"}`

[build-context] 다음 Phase 준비
- 워크트리 정리 (worktreePath가 있는 경우):
  `echo '{"projectId":"{ID}","phase":{N},"merge":true}' | node ... remove-worktree`
  → worktree 브랜치를 메인에 머지 후 정리. 실패 시 graceful skip
- `echo '{"completedTasks":[...]}' | node ... build-phase-context` → `{phaseContext:"..."}`
- advance: `{"completedAction":"build-context"}` → 다음 Phase 또는 complete

**주의사항:**
- project.json 직접 Read 금지 — next-step이 action/phase/tasks 등 필요 정보를 모두 반환
- execute.md Read 금지 — 이 레시피가 자체 완결적
- 프로젝트 파일 전체 Read 금지 — 태스크 설명에 명시된 파일만 읽을 것
- 빌드 에러 일괄 수정 — 에러가 여러 개면 한 번에 모두 수정 (반복 빌드 방지)
- 큰 JSON은 Write tool → /tmp/gv-*.json → --input-file로 전달

반환 형식: Phase별 요약 (각 300자 이내)
- 실행 태스크 수, 주요 결과물, 품질 검증 결과, 생성 파일 수, 수정 시도 (있는 경우)

에스컬레이션 발생 시: 즉시 에스컬레이션 정보 반환
confirm-next-phase 발생 시: 현재 Phase 완료 표시 후 반환

제외: 태스크별 상세 출력 전문, 리뷰 코멘트 전체, 중간 fix 시도 상세
...
Step 1: 프로젝트 로드 + 모드 선택 (메인 세션 — 조회 CLI + AskUserQuestion)
  ↓
Step 2: 실행 루프 (Task tool — 첫 Phase에서 init 포함 + Phase 실행 + 리뷰 + 품질 검증 + 수정)
  ↓ (에스컬레이션 발생 시)
...
- 실행 초기화 (첫 Phase에 포함: init-execution + update-status + execution-plan)
