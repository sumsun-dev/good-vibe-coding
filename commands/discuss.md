...
6. 전체 에이전트 병렬 리뷰 (`review-prompt`)
7. 수렴 확인 (`check-convergence`)
8. 이전 라운드 추이 분석 (previousRounds 전달)
9. 라운드 데이터 저장 (`add-discussion-round`)
...
**반환 크기 제한:** 기획서는 5000자 이내, 에이전트 출력은 각 1000자 이내, 리뷰는 각 300자 이내로 요약. 상세는 project.json에 저장하세요.
...
4. **기술 스택 요약** — 프레임워크, DB, 외부 서비스
5. **작업 규모** — 예상 Phase 수, 태스크 수
6. **수렴 상태** — 승인율, 블로커
