...
- **다음 단계:** `good-vibe:approve` (plan-only). plan-execute 모드에서는 good-vibe:new가 CTO+PO 빠른 분석을 자동 실행합니다.
...
## Step 2: 토론 + 리뷰 + 수렴 (Task tool 1회)

**Thin Controller 원칙:** 토론(Tier별 분석 + 종합) + 리뷰 + 수렴 확인을 하나의 Task tool로 실행합니다.
...
6. 핵심 리뷰어 선정 (`select-discussion-reviewers`) — 전원이 아닌 핵심 2-3명만 리뷰 (CTO, QA, Security 우선)
7. 선정된 리뷰어만 병렬 리뷰 (`review-prompt`) — **haiku 모델** 사용 (리뷰는 approve/reject 판단이므로 경량 모델 충분)
8. 수렴 확인 (`check-convergence`)
9. 조기 수렴 탈출: 라운드 2+에서 승인율 개선폭 < 5%이고 critical 블로커 0개면 즉시 수렴 처리
10. 이전 라운드 추이 분석 (previousRounds 전달)
11. 라운드 데이터 저장 (`add-discussion-round`)
...
5. 전체 팀원이 기획서를 리뷰합니다 (병렬, review-prompt)
6. 리뷰 결과를 분석하여 수렴 여부를 판단합니다 (check-convergence)
7. 이전 라운드 정보가 있으면 진화 추이를 계산합니다
8. 라운드 데이터를 저장합니다 (add-discussion-round)
...
**반환 크기 가이드:** 기획서(synthesizedPlan)는 전문을 그대로 반환하세요 (Mermaid 다이어그램, 아키텍처, 작업 분배 포함). 에이전트 출력은 각 역할의 핵심 결정/제안/리스크를 구조화하여 반환하세요 (원문 절삭 금지, 핵심 구조 유지). 리뷰는 verdict + 주요 피드백을 포함하세요. 상세 원문은 project.json에도 저장하세요.
...
- add-discussion-round: 라운드 데이터 저장
...
## Step 3: CEO 확인 (Ralph Loop)

**메인 세션 역할:** Step 2의 Task tool 결과를 CEO에게 표시하고, 피드백을 수집합니다.

### 표시 내용

Task tool 반환값에서 다음을 **전체** 표시합니다 (요약/축약 금지):

1. **기획서 전문** — synthesizedPlan을 그대로 표시합니다. 다음 섹션이 모두 포함되어야 합니다:
   - 프로젝트 개요 + 목표
   - 기술 아키텍처 (Mermaid 다이어그램 포함)
   - 화면 구조/흐름 (Mermaid flowchart, UI 프로젝트인 경우)
   - 와이어프레임 (ASCII art, UI 프로젝트인 경우)
   - 역할별 작업 분배
   - 일정 (마일스톤/Phase)
   - 기술 스택 + 외부 서비스
   - 리스크 및 대응
   - 미합의 사항 / CEO 결정 필요 사항
2. **팀원별 핵심 인사이트** — 각 에이전트의 고유 관점 요약 (역할명 + 핵심 제안 1-3줄)
3. **수렴 상태** — 승인율, 블로커, 진화 추이
...
AskUserQuestion:
  ...
  header: "기획 확인"
  options:
    - "이대로 진행 (Recommended)" — 기획서를 저장하고 다음 단계로 넘어갑니다
...
```
AskUserQuestion:
  질문: "기획서가 아직 수렴하지 않았습니다 (승인율 {N}%). 어떻게 할까요?"
  header: "미수렴"
  options:
    - "추가 토론 (Recommended)" — 피드백을 반영하여 다음 라운드를 진행합니다
    - "이대로 확정" — 현재 기획서로 확정하고 미합의 사항을 기록합니다
    - "수정 요청" — CEO 피드백을 추가하여 다음 라운드를 진행합니다
    - "중단" — 토론 종료, 기획서 미확정
```
...
#### "이대로 진행" / "이대로 확정" / "강제 수렴" 선택 시

Step 4(기획서 저장)로 넘어갑니다.

#### "수정 요청" 선택 시

CEO에게 수정 사항을 자유 입력받습니다.

입력받은 피드백을 `ceoFeedback`으로 저장하고, Step 2 Task tool을 다시 호출합니다 (피드백 포함).

이 루프는 CEO가 확정을 선택할 때까지 반복합니다.
CEO의 수정 요청 반복 횟수에 제한은 없습니다 (CEO 주도 종료, 수렴 라운드와는 별개).

#### "추가 토론" 선택 시

Step 2 Task tool을 다시 호출합니다 (feedbackForMe 주입).

#### "처음부터 다시" 선택 시

CEO에게 새로운 방향을 입력받고, Step 2 Task tool을 다시 호출합니다 (ceoFeedback 초기화).
...
```
참고: plan-execute 모드에서는 good-vibe:new가 CTO+PO 빠른 분석을 자동으로 실행합니다.
good-vibe:discuss를 직접 실행한 경우에도 기획서는 사용 가능합니다.
...
`good-vibe:new`에서 Task tool로 호출된 경우, 위 Step 1-5를 동일하게 실행하되 **최종 반환 형식**을 제한합니다.
...
Step 1: 프로젝트 로드 (메인 세션 — 단순 조회 CLI)
  ↓
Step 2: 토론 + 리뷰 + 수렴 (Task tool 1회 — Tier별 분석 + 종합 + 리뷰 + 수렴 확인)
  ↓
Step 3: CEO 확인 (메인 세션 — 기획서 + 수렴 상태 표시 + AskUserQuestion)
  ↓ (수정/추가토론 시 Step 2로 돌아가서 ceoFeedback/feedbackForMe 주입)
  ↓ (확정 선택 시)
Step 4: 기획서 저장 (메인 세션 — 단순 상태 업데이트 CLI)
  ↓
Step 5: 다음 단계 안내 (메인 세션 — 모드별 안내 표시)
```

**메인 세션 역할:**

- 프로젝트 조회 (list-projects)
- Task tool 결과 표시 (기획서 + 수렴 상태)
